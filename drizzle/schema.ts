...
export const orders = pgTable("orders", {
  ...
  // Partner referral-link token (partner username) for origin measurement, even without a coupon.
  partnerRef: varchar("partnerRef", { length: 100 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
export const coupons = pgTable("coupons", {
  ...
  isActive: boolean("isActive").notNull().default(true),
  // Internal first-purchase-only coupon (e.g. PRIMEIRACOMPRA): valid only for phones with no cadastro.
  firstPurchaseOnly: boolean("firstPurchaseOnly").notNull().default(false),
